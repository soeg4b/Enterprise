...
export default function FiberProjectDetailPage() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      const map = L.map(mapRef.current).setView(
        [project.nearEnd.latitude, project.nearEnd.longitude], 16,
      );
      ...
      L.marker([project.nearEnd.latitude, project.nearEnd.longitude], { icon: neIcon })
        .bindPopup(`<b>${project.nearEnd.name}</b><br/>${project.nearEnd.address ?? ''}`).addTo(map);
      L.marker([project.farEnd.latitude, project.farEnd.longitude], { icon: feIcon })
        .bindPopup(`<b>${project.farEnd.name}</b><br/>${project.farEnd.address ?? ''}`).addTo(map);
    ...
    return () => {
      cancelled = true;
      if (stateRef.current?.map) { stateRef.current.map.remove(); stateRef.current = null; }
    };
  }, [project]);
  ...
  async function handleUpload(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const r = await fetch(`${API_URL}/v1/fiber-projects/${id}/photos`, { method: 'POST', body: fd });
      const j = await r.json();
      if (!r.ok) { setUploadMsg(`Upload failed: ${j.detail ?? j.code ?? r.status}`); }
      else { setUploadMsg(`Pole #${j.pole.sequence} added. GPS source: ${j.gpsSource.toUpperCase()}.`); form.reset(); await reload(); }
    } catch (err) { setUploadMsg(`Error: ${String(err)}`); }
  ...
  async function handleOtdrUpload(e: React.FormEvent<HTMLFormElement>) {
    ...
    setOtdrUploading(true); setOtdrMsg(null);
    try {
      const fd = new FormData();
      fd.append('photo', file);
      ...
      if (r.status === 422 && j.code === 'OTDR_PHOTO_REJECTED') {
        setOtdrMsg({ kind: 'err', text: 'Foto OTDR ditolak — silakan ambil ulang foto.', failures: j.failures });
        return;
      }
      if (!r.ok) { setOtdrMsg({ kind: 'err', text: `Upload gagal: ${j.detail ?? j.code ?? r.status}` }); return; }
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 md:p-6 space-y-4">
      <div className="flex justify-between items-start flex-wrap gap-2">
        <div>
          <Link href="/fiber" className="text-xs text-sky-600 underline">← All Fiber Projects</Link>
      ...
      <div className="grid md:grid-cols-2 gap-3">
        <EndpointCard title="Near End (NE)" badge="NE" color="bg-green-600" data={project.nearEnd} />
        <EndpointCard title="Far End (FE)" badge="FE" color="bg-red-600" data={project.farEnd} />
      </div>
        ...
        <div className="bg-white rounded-lg border border-slate-200">
          ...
          <form onSubmit={handleUpload} className="p-4 space-y-3 text-sm">
            ...
            <div>
              ...
              <input name="uploadedBy" type="email" placeholder="mitra@vendor.co.id" defaultValue="mitra@demo" className="w-full border border-slate-300 rounded px-2 py-1" />
            </div>
            <div>
              <label className="block text-xs text-slate-600 mb-1">Note</label>
              <input name="note" type="text" placeholder="Pole condition / remarks" className="w-full border border-slate-300 rounded px-2 py-1" />
            </div>
            <button type="submit" disabled={uploading} className="w-full bg-slate-900 text-white py-2 rounded disabled:opacity-50">
        ...
        <div className="bg-white rounded-lg border border-slate-200">
          ...
          <details className="border-b border-slate-100">
            ...
            <form onSubmit={handleOtdrUpload} className="px-4 py-3 grid md:grid-cols-3 gap-3 text-xs bg-slate-50">
              <label className="space-y-1"><div className="text-slate-600">Segment</div><input name="segment" defaultValue="A-B" required className="w-full border border-slate-300 rounded px-2 py-1" /></label>
              <label className="space-y-1"><div className="text-slate-600">Core</div>
              ...
              <label className="space-y-1"><div className="text-slate-600">SOW Number</div><input name="sowNumber" defaultValue="SOW-PPO21-201-B" className="w-full border border-slate-300 rounded px-2 py-1" /></label>
              <label className="space-y-1"><div className="text-slate-600">Wavelength (nm)</div>
                <select name="wavelengthNm" required defaultValue="1310" className="w-full border border-slate-300 rounded px-2 py-1">
                  <option value="1310">1310</option><option value="1550">1550</option><option value="1625">1625</option>
                </select>
              </label>
              <label className="space-y-1"><div className="text-slate-600">Pulse Width (ns)</div><input name="pulseWidthNs" type="number" min="0" defaultValue="100" required className="w-full border border-slate-300 rounded px-2 py-1" /></label>
              <label className="space-y-1"><div className="text-slate-600">Scan Range (km)</div><input name="scanRangeKm" type="number" step="0.01" defaultValue="5" required className="w-full border border-slate-300 rounded px-2 py-1" /></label>
              <label className="space-y-1"><div className="text-slate-600">Total Length (km)</div><input name="totalLengthKm" type="number" step="0.0001" required className="w-full border border-slate-300 rounded px-2 py-1" /></label>
              <label className="space-y-1"><div className="text-slate-600">Total Loss (dB)</div><input name="totalLossDb" type="number" step="0.01" required className="w-full border border-slate-300 rounded px-2 py-1" /></label>
              <label className="space-y-1"><div className="text-slate-600">Device Model</div><input name="deviceModel" placeholder="e.g. EXFO MAX-730C" className="w-full border border-slate-300 rounded px-2 py-1" /></label>
              <label className="space-y-1 md:col-span-2"><div className="text-slate-600">Operator (email)</div><input name="operator" type="email" defaultValue="mitra@demo" className="w-full border border-slate-300 rounded px-2 py-1" /></label>
              <label className="space-y-1"><div className="text-slate-600">Notes</div><input name="notes" className="w-full border border-slate-300 rounded px-2 py-1" /></label>
                ...
                {otdrMsg && (
                  <div className={`text-xs ${otdrMsg.kind === 'ok' ? 'text-emerald-700' : 'text-rose-700'}`}>
                    ...
                    {otdrMsg.failures && otdrMsg.failures.length > 0 && (
                      <ul className="list-disc pl-5 mt-1">{otdrMsg.failures.map((f, i) => <li key={i}>{f}</li>)}</ul>
                    )}
                  </div>
                )}
              </div>
              ...
              {project.poles.map((p) => (
                <tr key={p.id} className={`border-t border-slate-100 ${highlightId === p.id ? 'bg-sky-50' : ''}`}>
                  ...
                  <td className="px-3 py-2">
                    ...
                    <img src={`${API_URL}${p.photoUrl}`} alt={`Pole ${p.sequence}`} className="w-16 h-16 object-cover rounded cursor-pointer hover:ring-2 hover:ring-sky-500" onClick={() => setLightbox(p)} />
                  </td>
        ...
        <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4" onClick={() => setLightbox(null)}>
          <div className="bg-white rounded-lg max-w-4xl w-full max-h-[90vh] overflow-hidden flex flex-col" onClick={(e) => e.stopPropagation()}>
            <div className="flex justify-between items-center px-4 py-2 border-b border-slate-200">
              ...
              <button onClick={() => setLightbox(null)} className="text-slate-500 hover:text-slate-900 text-2xl leading-none px-2" aria-label="Close">×</button>
            </div>
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src={`${API_URL}${lightbox.photoUrl}`} alt={`Pole ${lightbox.sequence}`} className="flex-1 object-contain bg-slate-100 min-h-0" style={{ maxHeight: 'calc(90vh - 120px)' }} />
...
function EndpointCard({ title, badge, color, data }: {
  ...
  return (
    <div className="bg-white p-3 rounded border border-slate-200 flex items-start gap-3">
      ...
      <div className="flex-1">
        ...
        <div className="font-mono text-[11px] text-slate-600 mt-1">{data.latitude.toFixed(6)}, {data.longitude.toFixed(6)}</div>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="p-4 grid md:grid-cols-[160px_1fr] gap-4">
      <div>
        {t.photoUrl ? (
          // eslint-disable-next-line @next/next/no-img-element
          <img src={`${API_URL}${t.photoUrl}`} alt="OTDR screen" className="w-full h-32 object-cover rounded border border-slate-200 cursor-zoom-in" onClick={() => window.open(`${API_URL}${t.photoUrl}`, '_blank')} />
        ) : (
      ...
      <div className="space-y-2">
        <div className="flex items-start justify-between flex-wrap gap-2">
          <div>
            ...
            <div className="text-sm font-semibold mt-0.5">{t.input.wavelengthNm} nm · pulse {t.input.pulseWidthNs} ns · range {t.input.scanRangeKm} km</div>
          </div>
        ...
        {a.notes.length > 0 && (
          <ul className="text-[11px] text-slate-600 list-disc pl-5 space-y-0.5">{a.notes.map((n, i) => <li key={i}>{n}</li>)}</ul>
        )}
        <div className="overflow-x-auto">
          <table className="w-full text-xs">
            <thead className="bg-slate-50 text-slate-600">
              <tr>
                <th className="px-2 py-1 text-left">#</th><th className="px-2 py-1 text-left">Type</th>
                <th className="px-2 py-1 text-right">Loc (km)</th><th className="px-2 py-1 text-right">Loss (dB)</th>
                <th className="px-2 py-1 text-right">Atten (dB/km)</th><th className="px-2 py-1 text-right">ORL (dB)</th>
            ...
            <tbody>
              {a.events.map((e) => (
                <tr key={e.index} className="border-t border-slate-100">
                  <td className="px-2 py-1">{e.index}</td><td className="px-2 py-1">{e.type}</td>
